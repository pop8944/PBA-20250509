...
namespace FrameworkOnnxTest
{
    public partial class IFOnnxRuntimeControl : UserControl
    {
        ...
        {
            eyeD.RemoveModel(DgvModel.SelectedRows[1].Cells[0].Value.ToString().Trim());
            RefreshModelList();
        }
        ...
        private void RefreshModelList()
        {
            DgvModel.Rows.Clear();
            int index = 0;
            foreach (string modelName in eyeD.Models.Keys)
            {
                DgvModel.Rows.Add(new object[] { index, modelName });
                index++;
            }
        }
    }
