...
using static IntelligentFactory.CSQLite;

namespace IntelligentFactory
{
    public partial class FormMenu_Viewer_Temp : Form
    {
        ...
        public FormMenu_Viewer_Temp(string str_BufferID, string strQrcodes, int nRowIndex)
        {
            InitializeComponent();
        ...
        public FormMenu_Viewer_Temp()
        {
            InitializeComponent();
            g_SelectGB = g_SelectGBM = 1;
        }

        public FormMenu_Viewer_Temp(FormMenu_MainFrame m_form)
        {
            InitializeComponent();
            frmmain = m_form;
        }

        public FormMenu_Viewer_Temp(bool RMSMODE)
        {
            InitializeComponent();
            RMS_Mode = RMSMODE;
        }

        public FormMenu_Viewer_Temp(string BUFFER_DATA)
        {
            InitializeComponent();
        ...
        public void InitGridSQL()
        {
            ...
            Dictionary<string, HistoryData> data = sqlite.SelectAll_DATE_HISTORY("HISTORY", time);

            gridHistory.Rows.Clear();

            foreach (var item in data)
            {
                HistoryData history = item.Value;
                gridHistory.Rows.Add(new string[] { history.DateTime, history.Model, history.QrCode, history.InspJudge, history.RmsJudge, history.MasterImagePath, history.NgImagePath, history.NgCount.ToString(), history.TactTime });
            }
        }
            ...
            try
            {
                ...
                if (gridHistory.SelectedRows.Count > 0 && nSelect_Index >= 0)
                {
                    ...
                    try
                    {
                        ...
                        lblNgPartsCount.Text = selectRow_Data[7];
                    }
                    catch { }
                ...
                if (Global.Device.NGBUFFER != null)
                {
                    ...
                    if (NgBufferIds.Count > 0)
                    {
                        ...
                        for (int i = 0; i < gridBufferMonitor.Rows.Count - 1; i++)
                        {
                            ...
                            {
                                gridBufferMonitor.Rows[i].DefaultCellStyle.BackColor = Color.IndianRed;
                                gridBufferMonitor.Rows[i].DefaultCellStyle.SelectionBackColor = Color.IndianRed;
        ...
        private void Buffer_DISP()
        {
            if (Global.Device.NGBUFFER != null && Global.Device.NGBUFFER.IsOpen)
            {
                ...
                for (int i = 0; i < gridBuffer.Rows.Count; i++)
                {
                    ...
                    {
                        gridBuffer.Rows[i].DefaultCellStyle.BackColor = Color.IndianRed;
                        gridBuffer.Rows[i].DefaultCellStyle.SelectionBackColor = Color.IndianRed;
                    }
                    else
                    {
                ...
                // 광주 사이트에서만 디스플레이..
                if (Global.Instance.Mode.NG_Buffer_Type)
                {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int NGcnt = 0;
                int OKcnt = 0;
                int Totalcnt = gridHistory.Rows.Count;

                for (int i = 0; i < gridHistory.Rows.Count; i++)
                {
                    if (gridHistory.Rows[i].Cells[3].Value.ToString() == "OK")
                    {
                        OKcnt++;
                    }
                    else if (gridHistory.Rows[i].Cells[3].Value.ToString() == "NG")
                    {
                        NGcnt++;
                    }
                }
        ...
        private void btnRmsJudgeOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Global.DB.UpdateJudgeNReason("HISTORY", "OK", true, g_SelDate, g_Part, strReason, bQRAll, strQrCode);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnRmsJudgeNG_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(strQrCode) == false)
                {
                    Global.DB.UpdateJudgeNReason("HISTORY", "NG", true, g_SelDate, g_Part, strReason, bOk, strQrCode);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnRmsJudgeIDLE_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(strQrCode) == false)
                {
                    Global.DB.UpdateJudgeNReason("HISTORY", "IDLE", true, g_SelDate, g_Part, strReason, bOk, strQrCode);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                for (int i = 0; i < Global.Instance.Device.NGBUFFER.GetBufferCount(); i++)
                {
                    ...
                    if (ids.Length > 0)
                    {
                        ...
                        for (int k = 0; k < gridBufferMonitor.Rows.Count; i++)
                        {
                            string QRCode = gridBufferMonitor[2, k].Value.ToString();

                            NgBufferIds.Add(QRCode);

                            for (int j = 1; j < strParse.Length; j++)
                            {
                                QRParser qrAdd = new QRParser(QRCode + strParse[i], true);
        ...
        {
            if (tcJobType.SelectedTab.Text == "History")
            {
